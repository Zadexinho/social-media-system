/**
 * Airtable Automation Script - Share Now (Updated for unified table)
 * Inputs: record_id, table_id
 * Now includes Client column filtering
 */
...
const webhookUrl = 'https://n8n.noesisautomation.com/webhook-test/share-now_dev';
const payload = {
  table_id: tableId,
  record_id: recordId,
  client_name: clientName, // NEW: Include client name
  action: 'posting',
  triggered_by: 'share_now',
  execution_type: 'immediate', // NEW: Distinguish from scheduled
...
try {
  ...
  if (response.ok) {
    console.log(`✅ Successfully sent record ${recordId} to n8n for immediate sharing (Client: ${clientName})`);
  } else {
